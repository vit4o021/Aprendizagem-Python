def estoque(*args):
    '''O primeiro for irá Percorrer cada uma das listas e segundo for irá percorrer cada uma das tuplas.
    Em seguida, será adicionado na lista de produtos somente os códigos dos produtos , o set irá retirar valores duplicados
     e por fim o programa retornará uma lista de produtos únicos e o tamanho
     '''
    lista_produtos = []
    set_produto = {}
    produtos_unicos = 0
    for lista in args:
        for produto, estoqu in lista:
            lista_produtos.append(produto)
            set_produto = set(lista_produtos)
            produtos_unicos = list(set_produto)
...
print(len(estoque(estoque_fim_jan, estoque_fim_fev, estoque_fim_mar, estoque_fim_abr)))
